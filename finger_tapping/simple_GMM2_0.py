...
from finger_tapping.pca_ica    import run_ica


def simple_GMM(subject_id: str, n_components_ica: int = 5, sampling_rate_hz: float = 7.81, random_state: int = 42, include_hbr: bool = False, plot: bool = True) -> tuple:
    """
    ...
    ica, df = run_ica(epochs, n_components=n_components_ica, random_state=random_state, include_hbr=include_hbr)

    # Renaming columns to IC1, IC2, ..., if not already named
    if not any(isinstance(c, str) and c.startswith("IC") for c in df.columns):
        df = df.rename(columns={col: f"IC{idx+1}"
