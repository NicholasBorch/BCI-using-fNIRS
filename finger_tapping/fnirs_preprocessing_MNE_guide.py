...
Link: https://mne.tools/stable/auto_tutorials/preprocessing/70_fnirs_processing.html
"""
...
### Added to guide to use BIDS formatted data
...
# Load the Finger Tapping dataset in BIDS format
bids_root = mne_nirs.datasets.fnirs_motor_group.data_path()

mne_bids.stats.count_events(bids_root)

# Select a subject (e.g., sub-01) and specify datatype as "nirs"
bids_path = BIDSPath(subject="01", task="tapping", datatype="nirs", root=bids_root)

# Load fNIRS data in SNIRF format
...
raw_intensity.annotations.rename(
    {"Control": "Control", "Tapping/Left": "Tapping/Left", "Tapping/Right": "Tapping/Right"}
)
...
### Orignal code: Does not work properly
...
### Fix of original code added
fig, axes = plt.subplots(nrows=2, ncols=2, figsize=(15, 6))

evoked_control = epochs["Control"].average()
evoked_tapping = epochs["Tapping"].average()

clims = dict(hbo=[-20, 20], hbr=[-20, 20])

# Plot Control condition
evoked_control.plot_image(
    axes=[axes[0, 0], axes[1, 0]],
    clim=clims,
    show=False
)

# Plot Tapping condition
evoked_tapping.plot_image(
    axes=[axes[0, 1], axes[1, 1]],
    clim=clims,
    show=False
)

axes[0, 0].set_title("Control: Oxyhemoglobin")
axes[1, 0].set_title("Control: Deoxyhemoglobin")
axes[0, 1].set_title("Tapping: Oxyhemoglobin")
axes[1, 1].set_title("Tapping: Deoxyhemoglobin")
