from finger_tapping.feature_preparation2_0 import extract_X_y
...
def run_pca(subject, n_components = 5, random_state = 42, include_hbr=False):
    ...
    """
    X, y, idx = extract_X_y(subject, include_hbr=include_hbr)
...
def run_ica(subject, n_components = 5, random_state = 42, include_hbr=False, return_index=True):
    """
    ...
    # Prepare DataFrame
    ica_df = pd.DataFrame(X_ica, index=idx) if idx is not None else pd.DataFrame(X_ica)
    ica_df["label"] = y
    return ica, ica_df
